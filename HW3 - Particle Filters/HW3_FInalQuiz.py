...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        alpha = random.gauss(motion[0], self.steering_noise) # add noise to turn
        d =  random.gauss(motion[1],self.distance_noise) # add noise to distance moved

        beta = (d/length) * tan(alpha)

        if abs(beta) < 0.001:
            x_prime = x + d*cos(theta)
            y_prime = y + d*sin(theta)
            theta_prime = (theta + beta) % (2*pi)
        else:
            R = d / beta
            x_prime = x - sin(theta)*R + sin(theta+beta)*R
            y_prime = y + cos(theta)*R - cos(theta+beta)*R
            theta_prime = (theta + beta) % (2*pi)
    ...
    def sense(self, noise_on = 1):
        ...
        for L in landmarks:
            angle = atan2( (L[0] - self.y), (L[1] - self.x)) - self.orientation + (noise_on * random.gauss(0.0, self.bearing_noise))
            angle %= 2*pi
            Z.append(angle)
